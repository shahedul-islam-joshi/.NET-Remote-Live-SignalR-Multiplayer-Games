...
namespace NeonGrid.Hubs
{
    public class GameHub : Hub
    {
        ...
        public async Task Login(string username)
        {
            ...
            // Send back success + current leaderboard
        ...
        public async Task JoinGame(string gameId)
        {
            ...
            if (game != null)
            {
                ...
                // Remove game from lobby list (it's now full)
                await RefreshLobby();
            }
        }
        ...
        public async Task MakeMove(string gameId, int index)
        {
            ...
            if (result.isValid)
            {
                ...
                if (result.isWin || result.isDraw)
                {
                    ...
                    await RefreshLobby(); // Update lobby to remove finished game if needed
                }
            }
        }
        ...
        {
            // Send list of games waiting for an opponent
            var openGames = _gm.Games
                .Where(g => g.PlayerO == null && !g.IsGameOver)
        ...
        {
            _gm.RemovePlayer(Context.ConnectionId);
